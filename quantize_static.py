...
def quantize_model(model, num_bits, device='cpu', exclude=None):
    """
    ...
    for name, module in model.named_modules():
        if isinstance(module, nn.Linear):
            ...
            weight_qparams = calculate_qparams(
                original_weight, num_bits=num_bits, flatten_dims=(1, -1), reduce_dim=None
            )
            quantized_weight = quantize(original_weight, qparams=weight_qparams)

            # quantize weights
            quantized_bias = None
            if module.bias is not None:
                quantized_bias = quantize(original_bias, num_bits=num_bits, flatten_dims=(0, -1))
    ...
    for name, quantized_weight, quantized_bias in to_be_quantized:
        if 'encoder' in name: # only quantize encoder
            ...
            setattr(layer_to_replace, last_attr_name, QLinear(quantized_weight.size(0), quantized_weight.size(1)))
